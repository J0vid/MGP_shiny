...
DO.size <- DO.gpa$size[names(DO.gpa$size) %in% dimnames(DO.probs)[[1]]]
DO.pheno <- DO.pheno[,,dimnames(DO.pheno)[[3]] %in% dimnames(DO.probs)[[1]]]


dim(DO.pheno)

DO.size <- DO.size[sort(names(DO.size), index.return = T)$ix]
...
View(cbind(dimnames(DO.pheno)[[3]], dimnames(DO.probs)[[1]], as.character(DO.covars$X), names(DO.size)))
...
pathway.loadings <- data.frame(gloadings = process.svd$mod$u[[1]], gnames = (as.character(rep(seq.info[,1], each = 8))), founders = rep(do.names, nrow(seq.info)))
...
proj.coords.a1 <- row2array3d(predict(process.svd, probs.rows[c(which.min(process.svd$mod$ts[[1]][,1]), which.max(process.svd$mod$ts[[1]][,1])),])$y, Nlandmarks = 54)
proj.coords.a2 <- proj.coords.a1[,,2]
proj.coords.a1 <- proj.coords.a1[,,1]
...
cor(process.svd$mod$V_super[,1], manova(two.d.array(tmp.mutant.registration) ~ c(rep(0, nrow(Y)), rep(1, sum(mutant.db$Genotype == mutant))))$coef[2,])
...
pathway.loadings <- data.frame(gloadings = random8state$mod$u[[1]], gnames = (as.character(rep(1:num.loci, each = 8))), founders = rep(do.names, num.loci))
...
chond.df <- data.frame(r2 = chond_perm)

ggplot(chond.df, mapping = aes(x = r2, fill = "#adcae6")) + 
  geom_histogram(alpha = 1, show.legend = FALSE) + 
  geom_vline(xintercept = true.r2.chond) +
  xlab(expression(paste("38 marker MGP R"^"2"))) +
  ylab("Frequency") +
  scale_fill_manual(values = "#adcae6") +
  theme_bw()

fgf.df <- data.frame(r2 = fgf_perm)

ggplot(fgf.df, mapping = aes(x = r2, fill = "#adcae6")) + 
  geom_histogram(alpha = 1, show.legend = FALSE) + 
  geom_vline(xintercept = true.r2.fgf) +
  xlab(expression(paste("41 marker MGP R"^"2"))) +
  ylab("Frequency") +
  scale_fill_manual(values = "#adcae6") +
  theme_bw()
...
```{r process correlation with PC1/2 and CAC}

giga.pca <- prcomp(Y)
DO.cac <- CAC(DO.pheno, DO.size)

process.correlations <- matrix(NA, ncol = 4, nrow = ncol(process.loadings))
process.correlations2 <- rep(NA, ncol(process.loadings))
cac.cor <- rep(NA, ncol(process.loadings))
...
for(i in 1:ncol(process.loadings)){
  ...
  process.correlations[i,1] <- cor(giga.pca$rotation[,1], process.loadings[,i])
  process.correlations[i,2] <- cor(giga.pca$rotation[,2], process.loadings[,i])
  process.correlations[i,3] <- cor(giga.pca$rotation[,3], process.loadings[,i])
  process.correlations[i,4] <- cor(giga.pca$rotation[,4], process.loadings[,i])
  print(i)
  # cac.cor[i] <- cor(DO.cac$CAC, process.loadings[,i])
}
...
process.cor.df <- data.frame(process.name = as.character(process.name), cor = abs(process.correlations))

process.cor.df[,1] <- as.character(process.cor.df[,1])

ggplot(process.cor.df) + 
  geom_polygon(aes(x = density(na.omit(cor.1))$x, y = density(na.omit(cor.1))$y), alpha = .1, show.legend = FALSE) + 
  # geom_polygon(aes(x = density(na.omit(cor.2))$x, y = density(na.omit(cor.2))$y), alpha = .1, show.legend = FALSE) + 
  xlab("Correlation to PC axis") +
  ylab("Density") +
  theme_bw()
...
plot(density(na.omit(process.cor.df$cor.1)), xlab = "Correlation to PC axis", ylab = "Density", main = "", ylim = c(0,11), xlim = c(0,1))
polygon(density(na.omit(process.cor.df$cor.2)))
polygon(density(na.omit(process.cor.df$cor.3)))
polygon(density(na.omit(process.cor.df$cor.4)))
...
mutant.lmsfin <- mutant.lms2[mutant.db2$Control == "No",]
mutant.dbfin <- mutant.db2[mutant.db2$Control == "No",]
...
```{r process mutant morphospace}
  ...
  tmp.mutant.registration <- gpagen(arrayspecs(rbind(Y, as.matrix(mutant.lmsfin[])), 54, 3))$coords
  tmp.mutant.mean <- gpagen(arrayspecs(rbind(Y, as.matrix(mutant.lmsfin)), 54, 3))$consensus
  ...
  mutant.mean.pc <- matrix(NA, nrow = length(unique(mutant.dbfin$Genotype)), ncol = 2)
  # for(i in c(1:5,7:10,13:14,22:23,28:29,34,36:37,41:44,46:48,50:52,63:65)){
   for(i in (1:length(unique(mutant.dbfin$Genotype)))){
    mutant <- unique(mutant.dbfin$Genotype)[i]
  mmean.score <- colMeans(mutant.scores[mutant.dbfin$Genotype == mutant,])
  text(mmean.score[1], mmean.score[2], labels = mutant, col = 2, cex = .7)
  mutant.mean.pc[i,2] <- mmean.score[2]
mutant.mean.pc[i,1] <- mmean.score[1]
  ...
  proc.score <- matrix(NA, nrow = 7, ncol = 3)
  
for(k in c(1:7)){
  tryCatch({
    process.name <- c("execution phase of apoptosis", "endochondral ossification", "bmp signaling pathway", "cranial suture morphogenesis", "wnt signaling pathway", "intraciliary transport", "neurotransmitter transport")[k]
  # process.ano <- as.character(unique(DO.go[,2])[sample(1:length(unique(DO.go[,3])), size = 1)])
  process.ano <- as.character(DO.go[DO.go[,3] == process.name,2])
    # process.name <- as.character(unique(DO.go[,3])[unique(DO.go[,2]) == process.ano])
#offline method for getting gene metadata
...
process.scores <- (full.pred) %*% tmp.pca$rotation
 # process.scores <- colMeans(process.scores)
  ...
  arrows(x0 = do.mean.proj[1], y0 = do.mean.proj[2], x1 = tmp.proc.score[1] + (tmp.proc.score[1] - do.mean.proj[1]) * mag, y1 = tmp.proc.score[2] + (tmp.proc.score[2] - do.mean.proj[2]) * mag, length = .05)
    ...
    proc.score[k,3] <-  tmp.proc.score[2] + (tmp.proc.score[2] - do.mean.proj[2]) * mag
proc.score[k,2] <- tmp.proc.score[1] + (tmp.proc.score[1] - do.mean.proj[1]) * mag
proc.score[k,1] <- process.name

if(k == 2 | k == 4){#flip these two vectors to point towards mutants
   proc.score[k,3] <-  tmp.proc.score[2] + (tmp.proc.score[2] - do.mean.proj[2]) * -(mag+1)
proc.score[k,2] <- tmp.proc.score[1] + (tmp.proc.score[1] - do.mean.proj[1]) * -(mag+1)
}
  ...
  #try it in ggplot
  mutant.mean.df <- data.frame(mutant = c(as.character(unique(mutant.dbfin$Genotype)), "DO mean"), scores = rbind(mutant.mean.pc, do.mean.proj[1:2]))
mutant.mean.df[,1] <- as.character(mutant.mean.df[,1])
colnames(mutant.mean.df) <- NULL
mutant.mean.df <- data.frame(mutant = c(mutant.mean.df[,1], proc.score[,1]), scores = rbind(as.matrix(mutant.mean.df[,-1]), cbind(as.numeric(proc.score[,2]), as.numeric(proc.score[,3]))), Effect = c(rep("Mutant", 30), rep("Genomic", 8)))
mutant.mean.df[,1] <- as.character(mutant.mean.df[,1])
mutant.mean.df[5,1] <- "B9D1"
mutant.mean.df[7,1] <- "Bmp2_7"
mutant.mean.df[9,1] <- "Bmp7 cond"
mutant.mean.df[10,1] <- "Crf4"
mutant.mean.df[11,1] <- "Disp"
mutant.mean.df[12,1] <- "Fgf10"
mutant.mean.df[13,1] <- "Fgf3"
mutant.mean.df[14,1] <- "Ghrhr"
mutant.mean.df[16,1] <- "Grm1 cond"
mutant.mean.df[17,1] <- "Hhat"
mutant.mean.df[18,1] <- "Ift88"
mutant.mean.df[19,1] <- "" #Lrp2
mutant.mean.df[20,1] <- "Mceph"
mutant.mean.df[22,1] <- "Nipbl"
mutant.mean.df[24,1] <- "Nosip hom"
mutant.mean.df[25,1] <- "" #Nosip het
mutant.mean.df[26,1] <- "Papps2"
mutant.mean.df[27,1] <- "Pten het"
mutant.mean.df[28,1] <- "Pten hom"
mutant.mean.df[29,1] <- "Shh"
mutant.mean.df[8,1] <- "" #Bmp7
mutant.mean.df[,2] <- as.numeric(mutant.mean.df[,2])
mutant.mean.df[,3] <- as.numeric(mutant.mean.df[,3])
#maybe color by type of mutant or pathway, or mutant vs genomic estimate
colnames(proc.score) <- c("process", "score1", "score2")
  
  
  
  
 # png("/mnt/Hallgrimsson/Users/Jovid/DOMGP/figures/morphospace_DOMGP2.png", width = 550, height = 350)  
  
  ggplot(mutant.mean.df, aes(x = scores.1, y = scores.2, label = mutant, color = Effect)) +
geom_point(colour = 1, size = .75) +
    stat_ellipse(data = mutant.mean.df[mutant.mean.df$Effect == "Genomic",], linetype = 2, alpha = .05, geom = "polygon") +
xlab("PC1 (11.2% var)") +
ylab("PC2 (3.9% var)") +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[1,2]), yend = as.numeric(proc.score[1,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[2,2]), yend = as.numeric(proc.score[2,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[3,2]), yend = as.numeric(proc.score[3,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[4,2]), yend = as.numeric(proc.score[4,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[5,2]), yend = as.numeric(proc.score[5,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[6,2]), yend = as.numeric(proc.score[6,3])), size = .4) +
geom_segment(aes(x = do.mean.proj[1], y = do.mean.proj[2], xend = as.numeric(proc.score[7,2]), yend = as.numeric(proc.score[7,3])), size = .4) +
ggrepel::geom_label_repel(segment.alpha = .2, size = 3.5, max.iter = 5000) +
...
#processes with multiple "large" effects: emt, bone development, forebrain, endochondral ossification, ap
...
#barplots of genetic effects
do.names <- c("A/J", "C57BL/6J", "129S1/SvImJ", "NOD/ShiLtJ", "NZO/HlLtJ", "CAST/EiJ", "PWK/PhJ", "WSB/EiJ")
do.colors <- c("A/J" = "#F0F000","C57BL/6J" = "#808080", "129S1/SvImJ"= "#F08080", "NOD/ShiLtJ" = "#1010F0","NZO/HlLtJ" = "#00A0F0","CAST/EiJ" = "#00A000", "PWK/PhJ" = "#F00000", "WSB/EiJ" = "#9000E0")

pathway.loadings <- data.frame(gloadings = pls.svd$mod$u[[1]], gnames = (as.character(rep(seq.info[,1], each = 8))), founders = rep(do.names, nrow(seq.info)))
...
#loop breaks on 13 & 14 because of / in the process name
pdf(paste0("mgp_barplots/", tree.processes[k], "_MGP.pdf"), width = 11.5, height = 5.5)
print(p)
dev.off()
...
heatmaply::heatmaply(tree.cor, symm = T, plot_method = "ggplot") #Rowv = NULL,
